...
class ImageCollection(object):
    ...
    Another use of ``load_func`` would be to convert all images to ``uint8``::
      ...
      ic = ImageCollection('/tmp/*go', load_func=imread_convert)
    ...
    >>> coll = io.ImageCollection(data_dir + '/chess*go')
    >>> len(coll)
    2
    >>> coll[0].shape
    (200, 200)

    >>> ic = io.ImageCollection(['/tmp/work/*go', '/tmp/other/*.jpg'])
    """
...
def imread_collection_wrapper(imread):
    def imread_collection(load_pattern, conserve_memory=True):
        ...
        load_pattern : str or list
            ...
            e.g. '/tmp/work/*go:/tmp/other/*.jpg'.  Also see
