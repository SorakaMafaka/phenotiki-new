    ...
    for filename in config_files:
        ...
        for p in provides:
            if p not in plugin_store:
                print("phenotiki `%s` wants to provide non-existent `%s`."
                      " Ignoring." % (name, p))
...
def call_plugin(kind, *args, **kwargs):
    ...
    kind : {'imshow', 'imsave', 'imread', 'imread_collection'}
        Function to look up.
    plugin : str, optional
        phenotiki to load.  Defaults to None, in which case the first
...
def use_plugin(name, kind=None):
    ...
    if kind is None:
        ...
        if kind not in plugin_provides[name]:
            raise RuntimeError("phenotiki %s does not support `%s`." %
                               (name, kind))
...
def _load(plugin):
    ...
    if plugin not in plugin_module_name:
        raise ValueError("phenotiki %s not found." % plugin)
    else:
    ...
    for p in provides:
        if p == 'imread_collection':
            ...
            print("phenotiki %s does not provide %s as advertised.  Ignoring." %
                  (plugin, p))
            continue
