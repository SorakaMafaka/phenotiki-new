...
def download_bin(plugin_names=["all"], package_dir=False):
    ...
    for plg in plugin_names:
        if plg not in PLUGINS_WITH_BINARIES:
            msg = "phenotiki {} not registered for binary download!".format(plg)
            raise Exception(msg)
...
def download_bin_main():
    ...
    phelp = (
        "phenotiki name for which to download the binary. "
...
def remove_bin_main():
    ...
    phelp = (
        "phenotiki name for which to remove the binary. "
